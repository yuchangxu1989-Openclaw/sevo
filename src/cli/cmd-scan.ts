...
import type { ScannerMode } from '../scan/tiered-scan-orchestrator.js';
...
export function registerScan(program: Command): void {
  program
    ...
    .option('--scanner <mode>', 'Scanner mode: llm (semantic, default) or filename (legacy)', 'llm')
    .option('--regenerate-map', 'Force regenerate sevo.scan.json LLM mapping', false)
    ...
    .action(async (opts: ScanOptions) => {
      ...
      try {
        ...
        const scannerMode: ScannerMode = (opts.scanner === 'filename' ? 'filename' : 'llm');
        ...
        const report = await orchestrator.run({
          ...
          l1: level === '1' || level === 'all' ? {
            ...
            testCommand: { command: 'npm test -- --run', cwd: root },
          } : undefined,
          ...
          scanner: scannerMode,
          regenerateMap: opts.regenerateMap,
        });
        ...
        if (opts.json) {
          ...
          if (report.l1) {
            const covered = report.l1.entries.filter((e) => e.status === 'covered').length;
            console.log(`  L1: ${covered}/${report.l1.entries.length} FRs covered`);
          }
          if (report.l2) {
            const covered = report.l2.entries.filter((e) => e.status === 'covered').length;
            console.log(`  L2: ${covered}/${report.l2.entries.length} ACs covered`);
          }
          if (report.l3) {
            const alive = report.l3.entries.filter((e) => e.status === 'alive').length;
            console.log(`  L3: ${alive}/${report.l3.entries.length} runtime checks alive`);
          }
