...
- **验收标准**：
  ...
  - AC-4.8v：任何架构设计产出完成后，必须同时经过 PM 评审和 Audit 评审并双重通过后，Architecture Design 阶段才能标记为 `passed` 并允许进入 Implement；PM 评审验证需求对齐，Audit 评审验证技术可行性与 spec 一致性。该准入要求适用于所有架构设计产出，不限于特定规模。Why：架构方案是 Implement 的准入输入，若方案存在需求理解偏差或技术不可行，后续实现会建立在错误基础上，修复成本远高于方案阶段发现。 验收验证：审计时按本条描述执行或复现对应操作，记录结构化结果 `{ acId, status, evidence, reason }`；`status` 必须为 `pass`，`evidence` 必须包含可观测输出（文件路径、CLI 输出、API 响应、页面截图、审计事件或状态字段之一），缺少证据、字段值不符或无法复现均判定为 `fail`。
...
- **处理**：作为 plan-review-gate（mandatory, never skip）执行，由产品视角、开发视角、质量视角、体验视角四方并行会审，检查需求承接完整度、实现可行性、决策严谨性、交互合理性、扩展边界和交付波次，并形成是否允许进入 Implement 的门禁结论。体验视角需核对契约与 UX 交互设计文档的一致性；开发视角需核对契约与架构详设文档的一致性。纯后端/CLI 项目可按项目配置省略 UX 视角，此时退化为三方会审，但 plan-review-gate 本身不可省略。
...
- **输入**：受管项目注册信息、真实流水线状态、阶段队列、阶段事件、阶段产物、FR 覆盖结果和阻塞原因。所有数据必须来自真实运行态或只读投影，禁止 demo、seed、mock 或前端自造数据。
- **处理**：
