...
优势：
- 不绕过 aco-dispatch-guard
- 主会话保持调度权，人类可干预
- 复用 aco-run-watchdog 已验证的 auto-advance notice 模式
...
2. aco-run-watchdog 的 auto-advance notice 已证明 before_prompt_build 注入是可靠的推进机制
3. 如果主会话忽略注入（L6 稀释），aco-run-watchdog 的 auto-advance 提供二级兜底
4. 未来 OpenClaw 如果暴露 spawn API，可以平滑升级到方案 A

## 后果
- 阶段推进有 1-2 轮对话的延迟（主会话需要被触发才能读到注入）
- 需要确保 before_prompt_build 的 priority 正确（850，在 aco-dispatch-guard 900 之后）
