...
用户通过 GitHub 仓库（yuchangxu1989-Openclaw/sevo）、ClawHub 插件市场或现有 OpenClaw 项目 README 中的引用发现 SEVO。获取方式为 `git clone` 仓库后运行安装脚本，或通过 ClawHub 一键安装（FR-D01）。安装命令自动检测 OpenClaw 版本兼容性，失败时输出人类可读错误信息和修复建议。

### 2. 安装与配置

运行 `bash init.sh` 一条命令完成全部安装：自动注册插件到 `openclaw.json plugins`、注册 `before_prompt_build` 和 `subagent_ended` hooks、检测当前 Agent 池并输出适配报告（FR-O12）。前置依赖仅 OpenClaw Gateway。安装完成后运行 `sevo:doctor` 验证配置完整性，输出 Errors / Warnings / OK 三级结果（FR-D07）。零额外配置即可运行。

### 3. 首次使用

用户执行 `sevo:create my-project "做什么"` 创建第一个流水线。Router 自动评估需求规模并选择档位（FR-D08）。创建成功后用户看到流水线 ID、阶段列表和第一个阶段的推进提示（[SEVO Auto-Advance]）。内置示例项目 hello-sevo 使用轻量流程（跳过三方会审和 UX 验收），5 分钟内完成 spec 产出并展示产出物清单和下一步建议（FR-D02）。

### 4. 核心流程

完整 Tier 3 流水线包含 15 个阶段：spec → spec-review-gate → test-case-authoring → contract → contract-review-gate → implement → review → regression → smoke-test → ux-acceptance → e2e-verification → publish-generalization-gate → deploy → verify → ledger。每个阶段完成后用户看到 [SEVO Auto-Advance] 提示，流水线自动派发下一阶段任务（FR-O05）。用户的核心动作是在方向性决策点做确认，日常推进由插件自动驱动。

三档路由（FR-D08）：Tier 1 直接执行琐碎操作不建流水线；Tier 2 走精简 5 阶段（spec → implement → review → verify → ledger）；Tier 3 走完整 15 阶段并创建独立项目文件夹。用户可通过 `--tier` 参数强制指定档位。

收敛循环（FR-D09）在关键阶段（Tier 3 的 publish-generalization-gate 后、Tier 2 的 review 后）自动触发终局差距分析，覆盖代码健壮性、文档-代码一致性、发布包完整性、陌生人 UX 走通、配置完整性 5 个维度。P0/P1 差距自动派修复并循环，最多 3 次。

### 5. 错误恢复

阶段失败时用户看到结构化诊断信息：错误码、描述、可能原因、建议操作（FR-D06）。用户可执行 `sevo:diagnose` 对失败流水线做深度诊断。自动重试策略（FR-O06）：同梯队失败 1 次后禁止原样重派，必须拆分或升级梯队。收敛循环超过 3 次仍有 P0 时暂停流水线，等待用户决策（FR-D09）。用户可通过 `sevo:pause` / `sevo:skip` / `sevo:retry` / `sevo:resume` 手动干预（FR-D03）。

### 6. 升级与维护

升级方式：`git pull` 拉取最新代码后重启 Gateway。升级前自动备份当前配置和活跃流水线状态（FR-D05）。`state/` 目录向后兼容，schema 变更时提供自动迁移。major 版本升级需用户确认。配置文件格式保持稳定，升级不破坏现有配置。
...
#### FR-D09: 自主收敛循环（Autonomous Convergence Loop）｜ 已实现(插件)

在 Tier 3 流水线的 `publish-generalization-gate` 阶段之后（或 Tier 2 的 `review` 阶段之后），自动触发一次终局差距分析。分析基准是项目创建时声明的终局目标（sevo:create 的 description 参数），而非仅 FR 列表。

差距分析维度：
1. 代码健壮性（错误处理、边界场景）
2. 文档-代码一致性（README 示例 vs 实际正则/API）
3. 发布包完整性（package.json / LICENSE / .gitignore）
4. 陌生人 UX 走通（clone → install → first-use → error-recovery）
5. 配置完整性（版本号一致、依赖声明）

行为：
- 差距分析产出报告，每个 gap 标注 P0/P1/P2
- P0/P1 存在 → 自动拆分修复任务并派发 → 修复完成后重新触发差距分析（循环）
- 仅 P2 及以下 → 记录到 ledger，流水线继续推进
- 最大循环次数：3（防止无限循环）
- 超过 3 次仍有 P0 → 暂停流水线，等待用户决策

**AC:**
- AC-D09-1: Tier 3 流水线在 publish-generalization-gate 后自动触发差距分析
- AC-D09-2: Tier 2 流水线在 review 后自动触发差距分析
- AC-D09-3: 差距分析基准包含项目 description，不仅是 FR 列表
- AC-D09-4: P0/P1 自动派修复并循环
- AC-D09-5: 最大循环 3 次，超限暂停
- AC-D09-6: 收敛后差距报告写入 ledger
- AC-D09-7: 差距分析必须覆盖以下 5 个维度：代码健壮性、文档-代码一致性、发布包完整性、陌生人 UX 走通、配置完整性
- AC-D09-8: 分析维度与「商用就绪定义」章节保持一致，任何一方变更时必须同步更新
...
## 六、商用就绪定义（Release Readiness Definition）

SEVO 满足以下全部条件才算可发布：

1. 代码健壮性：所有 JSON.parse 有 try-catch，文件 I/O 有错误处理，无未捕获异常路径
2. 文档-代码一致性：README 中所有命令示例与代码中正则/处理逻辑一致，零偏差
3. 发布包完整性：package.json（name/version/description/license/main）、LICENSE、.gitignore、README.md 齐全
4. 陌生人 UX 走通：一个从未见过 SEVO 的人，按 README 操作，能在 10 分钟内完成 clone → install → 创建第一个流水线 → 看到第一个阶段产出
5. 配置完整性：版本号在 package.json / README / init.sh 三处一致
6. 独立审计通过：至少一次由非开发 Agent 执行的完整审计，结果为 PASS
7. 双仓库同步：extensions/ 和独立仓库的关键文件 diff 为空

前 5 条是 FR-D09 收敛循环的自动分析维度，后 2 条（独立审计、双仓库同步）是发布门槛但需人工确认。7 条全部满足才算商用就绪。
