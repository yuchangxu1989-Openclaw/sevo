 ...
 * UFR-08: 重试失败 FR — 从最近失败的细阶段以 attempt+1 重新排队
 */

import { NextResponse } from 'next/server';
import { retryFr } from '@/lib/engine-service';
...
export async function POST(
  ...
  const result = retryFr(params.id, parsed.body);
  if (!result.success) {
    ...
    return errorResponse(status, code, result.error ?? 'Retry failed');
  }
